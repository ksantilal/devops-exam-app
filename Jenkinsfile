...
pipeline {
    ...
    environment {
        DOCKER_IMAGE = "kaapil2017/devopsexamapp:latest"
    }

    stages {
        stage('Git Checkout') {
            steps {
                git 'https://github.com/ksantilal/devops-exam-app.git', 
                    branch: 'master'
            }
        }
...
pipeline {
    ...
    environment {
        DOCKER_IMAGE = "kapil2017/devopsexamapp:latest"
    }
...
pipeline {
    ...
    environment {
        ...
        AWS_REGION = "eu-west-2"  // Update to your region
    }
